"""
Kaplan-Meier inverce censor weighted metrics.

TODO:
    Should be integrated (i.e. removed) as special case of inverce_censor_weight.
"""
...
def integrated_brier_score_km_numpy(times_grid, prob_alive, durations, events):
    ...
    b_scores = brier_score_km(times_grid, prob_alive, durations, events)
...
def integrated_brier_score_km(prob_alive_func, durations, events,
    ...
    return integrated_brier_score_km_numpy(times_grid, prob_alive, durations, events)
...
def integrated_binomial_log_likelihood_km_numpy(times_grid, prob_alive, durations, events):
    '''Compute the integrated brier score (for survival).
    This funcion takes pre-computed probabilities, while the function integrated_brier_score
    takes a function and a grid instead.
    ...
    assert pd.Series(times_grid).is_monotonic_increasing,\
        'Need monotonic increasing times_grid.'
    scores = binomial_log_likelihood_km(times_grid, prob_alive, durations, events)
