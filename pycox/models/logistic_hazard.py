...
class LogisticHazard(models.base._SurvModelBase):
    ...
    def interpolate(self,scheme='const_pmf', duration_index=None, sub=10):
        ...
        There are two schemes:
            ...
            `const_pmf` and `lin_surv` which assumes pice-wise constant pmf in each interval (linear survival).
        
        Keyword Arguments:
            scheme {str} -- Type of interpolation {'const_hazard', 'const_pmf'}.
                See `InterpolateDiscrete` (default: {'const_pmf'})
        ...
        return InterpolateDiscrete(self, scheme, duration_index)
...
class InterpolateDiscrete:
    ...
    Keyword Arguments:
        duration_index {np.array} -- Cuts used for discretization. Does not affect interpolation,
            only for setting index in `predict_surv_df` (default: {None})
        scheme {str} -- Type of interpolation {'const_hazard', 'const_pmf'} (default: {'const_pmf'})
        sub {int} -- Number of "sub" units in interpolation grid. If `sub` is 10 we have a grid with
            10 times the number of grid points than the original `duration_index` (default: {10}).
    
    Keyword Arguments:
    """
    def __init__(self, model, scheme='const_pmf', duration_index=None, sub=10, epsilon=1e-7):
        self.model = model
        self.scheme = scheme
        self.duration_index = duration_index
        self.sub = sub
    ...
    def predict_hazard(self, input, sub=None, batch_size=8224, numpy=None, eval_=True, to_cpu=False,
                       num_workers=0):
        if self.scheme in ['const_hazard', 'exp_surv']:
            haz = self._hazard_const_haz(input, sub, batch_size, numpy, eval_, to_cpu, num_workers)
        else:
            raise NotImplementedError
        return haz

    def predict_surv(self, input, sub=None, batch_size=8224, numpy=None, eval_=True, to_cpu=False, num_workers=0):
        if self.scheme in ['const_hazard', 'exp_surv']:
            surv = self._surv_const_haz(input, sub, batch_size, numpy, eval_, to_cpu, num_workers)
        elif self.scheme in ['const_pmf', 'lin_surv']:
            surv = self._surv_const_pmf(input, sub, batch_size, numpy, eval_, to_cpu, num_workers)
        else:
            raise NotImplementedError
        return surv

    def _surv_const_pmf(self, input, sub=None, batch_size=8224, numpy=None, eval_=True, to_cpu=False,
                        num_workers=0):
        if sub is None:
            sub = self.sub
    ...
    def _hazard_const_haz(self, input, sub=None, batch_size=8224, numpy=None, eval_=True, to_cpu=False,
                          num_workers=0):
        if sub is None:
            sub = self.sub
        ...
        n = haz.shape[0]
        haz = haz.view(-1, 1).repeat(1, sub).view(n, -1).div(sub)
        haz = pad_col(haz, where='start')
        return array_or_tensor(haz, numpy, input)

    def _surv_const_haz(self, input, sub=None, batch_size=8224, numpy=None, eval_=True, to_cpu=False,
                        num_workers=0):
        haz = self._hazard_const_haz(input, sub, batch_size, False, eval_, to_cpu, num_workers)
        surv = haz.cumsum(1).mul(-1).exp().transpose(0, 1)
        return array_or_tensor(surv, numpy, input)

    def predict_surv_df(self, input, sub=None, batch_size=8224, eval_=True, num_workers=0):
        if sub is None:
            sub = self.sub
        surv = self.predict_surv(input, sub, batch_size, True, eval_, num_workers)
        index = None
        if self.duration_index is not None:
            index = make_subgrid(self.duration_index, sub)
